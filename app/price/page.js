...
const th = { textAlign: "left", padding: "10px 12px", background: "#eef3f0", color: "#1a2b3c", fontSize: 13.5, fontWeight: 700, borderBottom: "1px solid #dce4df" };
...
export default function PricePage() {
  return (
    <main style={{ maxWidth: 800, margin: "0 auto", padding: "2.5rem 1.5rem 4rem" }}>
      ...
      <p style={{ letterSpacing: 2, fontSize: 12, color: "#0a7d3c", fontWeight: 700, margin: 0 }}>PRICE INDEX</p>
      <h1 style={{ fontSize: "1.9rem", lineHeight: 1.35, margin: "6px 0 12px", color: "#12263a" }}>
        太陽光・蓄電池の費用相場2026｜kW・容量別の目安
      </h1>
      <p style={{ color: "#44586b", lineHeight: 1.8, marginBottom: 8 }}>
      ...
      {/* 太陽光 */}
      <h2 style={{ fontSize: "1.3rem", color: "#12263a", margin: "0 0 6px" }}>太陽光発電の費用相場</h2>
      ...
      <p style={{ fontSize: 12, color: "#8a99a8", margin: "0 0 18px" }}>
        出典：経済産業省 調達価格等算定委員会 令和8年度意見（<a href={SOLAR_UNIT_SOURCE} target="_blank" rel="nofollow noopener noreferrer" style={{ color: "#0a7d3c" }}>meti.go.jp</a>）・確認日 {PRICE_CONFIRMED_AT}
      </p>
      <h3 style={{ fontSize: "1.05rem", color: "#1a2b3c", margin: "0 0 8px" }}>出力別の総額目安</h3>
      <Table head={["出力", "新築（28.9万円/kW・2025）", "既築（32.6万円/kW・2024）"]} rows={SOLAR_BY_OUTPUT.map((s) => [s.kw, s.newb, s.exist])} />
      <p style={{ fontSize: 12, color: "#8a99a8", margin: "0 0 30px" }}>※単価×出力の単純計算（税抜・補助金前）。容量が小さいほど固定費按分で割高になりやすい点にご留意ください。</p>

      {/* 蓄電池 */}
      <h2 style={{ fontSize: "1.3rem", color: "#12263a", margin: "0 0 6px" }}>家庭用蓄電池の費用相場</h2>
      ...
      <p style={{ fontSize: 12, color: "#8a99a8", margin: "0 0 18px" }}>
        出典：SII 公募要領（<a href={BATTERY_UNIT_SOURCE_SII} target="_blank" rel="nofollow noopener noreferrer" style={{ color: "#0a7d3c" }}>sii.or.jp</a>）・経済産業省 定置用蓄電システムの現状と課題（<a href={BATTERY_UNIT_SOURCE_METI} target="_blank" rel="nofollow noopener noreferrer" style={{ color: "#0a7d3c" }}>meti.go.jp</a>）・確認日 {PRICE_CONFIRMED_AT}
      </p>
      <h3 style={{ fontSize: "1.05rem", color: "#1a2b3c", margin: "0 0 8px" }}>容量別の総額目安</h3>
      <Table head={["容量", "総額目安（SII目標 12.5万円/kWh ベース）"]} rows={BATTERY_BY_CAPACITY.map((b) => [b.kwh, b.total])} />
      <p style={{ fontSize: 12, color: "#8a99a8", margin: "0 0 30px" }}>※SII目標価格（工事費込・税抜）×容量の単純計算。実勢は製品・全負荷/特定負荷・施工で変動します。補助金前の金額です。</p>

      {/* 売電・自家消費 */}
      <h2 style={{ fontSize: "1.3rem", color: "#12263a", margin: "0 0 6px" }}>売電・自家消費の考え方（卒FIT）</h2>
      ...
      <p style={{ fontSize: 12, color: "#8a99a8", margin: "0 0 30px" }}>
        出典：経済産業省 調達価格等算定委員会 令和8年度意見（<a href={SELL_SOURCE} target="_blank" rel="nofollow noopener noreferrer" style={{ color: "#0a7d3c" }}>meti.go.jp</a>）・確認日 {PRICE_CONFIRMED_AT}
      </p>

      {/* 運営・出典方針 */}
      <section style={{ marginTop: 20, borderTop: "1px solid #e3e9ee", paddingTop: 24 }}>
        <h2 style={{ fontSize: "1.05rem", color: "#1a2b3c", margin: "0 0 10px" }}>このデータについて（出典・更新方針）</h2>
        <ul style={{ color: "#44586b", fontSize: 14, lineHeight: 1.9, paddingLeft: "1.1rem", margin: 0 }}>
        ...
        <p style={{ marginTop: 18, display: "flex", gap: 18, flexWrap: "wrap" }}>
          <Link href="/subsidy/" style={{ color: "#0a7d3c", fontWeight: 700 }}>→ 補助金データベース（国＋自治体）</Link>
