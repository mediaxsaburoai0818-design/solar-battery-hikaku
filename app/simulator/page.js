...
export default function SimulatorPage() {
  return (
    <main style={{ maxWidth: 720, margin: "0 auto", padding: "2.5rem 1.5rem 4rem" }}>
      <p style={{ letterSpacing: 2, fontSize: 12, color: "var(--brand)", fontWeight: 700, margin: 0 }}>SIMULATOR</p>
      <h1 style={{ fontSize: "1.85rem", lineHeight: 1.35, margin: "6px 0 12px", color: "var(--ink)" }}>
        太陽光・蓄電池 投資回収シミュレーター
      </h1>
      <p style={{ color: "var(--ink-soft)", lineHeight: 1.8, marginBottom: 20 }}>
      ...
      {/* 前提と出典 */}
      <section style={{ marginTop: 30, borderTop: "1px solid var(--line)", paddingTop: 22 }}>
        <h2 style={{ fontSize: "1.05rem", color: "var(--ink)", margin: "0 0 10px" }}>試算の前提と出典</h2>
        <ul style={{ color: "var(--ink-soft)", fontSize: 13.5, lineHeight: 1.9, paddingLeft: "1.1rem", margin: 0 }}>
          ...
          <li>補助金：<Link href="/subsidy/" style={{ color: "var(--brand)" }}>補助金データベース</Link>の一次確認値（東京都・神奈川・埼玉・千葉 等・2026-07-25時点）を簡易モデル化。上限・条件（同時導入必須等）は各制度ページで要確認。</li>
        </ul>
        <p style={{ color: "var(--ink-faint)", fontSize: 12.5, lineHeight: 1.8, marginTop: 12 }}>
        ...
        <p style={{ marginTop: 16, display: "flex", gap: 18, flexWrap: "wrap" }}>
          <Link href="/price/" style={{ color: "var(--brand)", fontWeight: 700 }}>→ 費用相場（kW・容量別）</Link>
          <Link href="/subsidy/" style={{ color: "var(--brand)", fontWeight: 700 }}>→ 補助金データベース</Link>
